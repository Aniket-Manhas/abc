...
import { playAlertBeep, stopAlertBeep } from '../utils/audio';
...
export const SocketProvider = ({ children }) => {
  ...
  return (
    <SocketContext.Provider value={{
      ...
      setAdminAlerts, isAlarmActive, setIsMuted, isMuted
    }}>
      {children}
      {hasActiveDanger && (
        <div style={{
          position: 'fixed', top: 20, left: '50%', transform: 'translateX(-50%)',
          background: isMuted ? 'rgba(55, 65, 81, 0.95)' : 'rgba(239, 68, 68, 0.95)', 
          color: 'white',
          padding: '1rem 2rem', borderRadius: '50px', zIndex: 9999,
          display: 'flex', alignItems: 'center', gap: '1.5rem',
          boxShadow: '0 10px 25px rgba(0,0,0,0.3)', border: '2px solid rgba(255,255,255,0.2)',
          transition: 'all 0.3s ease'
        }}>
          <div style={{ fontWeight: 800, fontSize: '1.1rem', letterSpacing: '0.05em', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <span style={{ animation: isMuted ? 'none' : 'pulse 1s infinite' }}>🚨</span> 
            {isMuted ? 'STAMPEDE ALARM SILENCED' : 'STAMPEDE RISK DETECTED'}
          </div>
          <button 
            onClick={() => setIsMuted(!isMuted)}
            style={{
              background: 'white', color: isMuted ? '#374151' : '#ef4444', border: 'none',
              padding: '0.5rem 1.25rem', borderRadius: '25px', fontWeight: 700,
